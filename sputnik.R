...
## data from the paper
patient_groups <- read_tsv('age-groups.tsv',show_col_types=FALSE) %>%
  mutate(n_patients=n_treatment+n_control)

control_infections = 62
treatment_infections = 16
...
simGroup <- function(n_patients,infection_rates=infection_rate,treatment_prob = 0.75) { 
