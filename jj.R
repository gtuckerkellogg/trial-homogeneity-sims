...
## data from the paper
patient_groups <- read_tsv('age-groups - copy - copy - copy - copy.tsv',show_col_types=FALSE) %>%
  mutate(n_patients=n_treatment+n_control)

control_infections = 348
treatment_infections = 116
...
simGroup <- function(n_patients,infection_rates=infection_rate,treatment_prob = 0.5) { 
